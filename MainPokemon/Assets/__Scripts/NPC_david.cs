...
public class NPC_david : MonoBehaviour {
	...
	public void PlayDialog(){
		if (readyToBattle) {
			...
			BattleScreen.S.enemyName = "GYM RAT DAVID";
			
			Pokemon pkm1 = new Pokemon ("MEOWTH", "MOMO", 36, 52, 8, 14, 10, 8, 9, "NORMAL", "");
			pkm1.moves = new List<Move>();
			pkm1.moves.Add (Move.getMove("SCRATCH"));
			pkm1.moves.Add (Move.getMove("TAIL WHIP"));
			Pokemon pkm2 = new Pokemon ("PIKACHU", "PIKA", 44, 25, 9, 13, 14, 10, 13, "ELECTRIC", "");
			pkm2.moves = new List<Move>();
			pkm2.moves.Add (Move.getMove("TACKLE"));
			pkm2.moves.Add (Move.getMove("TAIL WHIP"));
			pkm2.moves.Add (Move.getMove("THUNDERSHOCK"));
